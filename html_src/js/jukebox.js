...
function Jukebox(element, opts) {
    ...
    this.joinChannel = function(channel) {
        _doAction(new Action("join_channel", {
            channel: channel
        }));
        return this;
    };
	...
	*/
    ...
    this.sendCreateAccountRequest = function(nick, pwd) {
        _doAction(new Action("create_user", {
            nickname: nick,
            password: pwd
        }));
        return this;
    ...
    this.sendChangePasswordRequest = function(nick, old_pwd, new_pwd, new_pwd2) {
        _doAction(new Action("change_user_password", {
            nickname: nick,
    ...
    this.deleteUploadedFile = function(filename) {
        ...
        if (Object.isArray(filename)) {
            for (var i = 0; i < filename.length; ++i) {
                action = new Action("delete_uploaded_file", {
    ...
    this.validateUploadedFile = function(filename) {
        if (Object.isArray(filename)) {
            for (var i = 0; i < filename.length; ++i) {
                _doAction(new Action("validate_uploaded_file", {
                    file_name: filename[i]
                }));
            }
        } else {
