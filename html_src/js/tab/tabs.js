...
this.Tabs = Class.create({
    ...
    addTab: function(tab, className) {
        ...
        var noHref = 'javascript:;',
            toggleTab = new Element('a', {
                href: noHref
            }).update('<span>' + tab.name + '</span>'),
            removeTab = new Element('a', {
                href: noHref
            }).update('<span> X </span>');
        ...
        var tabDisplay = new Element('li', {
            "class": this.rootCSS + '-tabHeader-' + id
        }).insert({
            top: toggleTab,
            bottom: removeTab
        });
        ...
        var tabContentContainer = new Element('div', {
            "class": this.rootCSS + '-tabContent-' + id
        });
        if (this.tabs.length > 1) {
